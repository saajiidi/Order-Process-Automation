...
def render_executive_summary(df_sales: pd.DataFrame, df_customers: pd.DataFrame, 
    ...
    month_revenue = pd.to_numeric(df_sales[df_sales[date_col] >= pd.Timestamp.now().replace(day=1)][revenue_col], errors='coerce').sum() if date_col in df_sales.columns else 0
